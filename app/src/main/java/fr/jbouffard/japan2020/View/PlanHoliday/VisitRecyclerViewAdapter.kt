...
class VisitRecyclerViewAdapter(private val visits: List<Visit>, private val listener: (Visit, Int) -> Unit) : RecyclerView.Adapter<VisitRecyclerViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: VisitRecyclerViewAdapter.ViewHolder, position: Int) = holder.bind(visits[position], listener, position)

    override fun getItemCount() = visits.size


    class ViewHolder(visitView: View) : RecyclerView.ViewHolder(visitView) {
        fun bind(visit: Visit, listener: (Visit, Int) -> Unit, position: Int) = with(itemView) {
            ...
            setOnClickListener { listener(visit, position) }
        }
    }
